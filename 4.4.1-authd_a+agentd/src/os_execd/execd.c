...
#ifndef WIN32
    // 如果比對後 command的名字事restart-wazuh的話
    if (!strcmp(name, "restart-wazuh")) {
        mdebug1("(execd) receive command restart-wazuh");
   ...
   // ################################################################################
    ...
    if (wfd) {
        ...
        if (timeout_value) {
            ...
            // JNote : Print debug log
            mdebug1("(execd) Check if this command was already executed");
            timeout_node = OSList_GetFirstNode(timeout_list);
            while (timeout_node) {
                timeout_data *list_entry;
                ...
                if (strcmp(list_entry->rkey, rkey) == 0) {
                    mdebug1("(execd) executed this command before and we don't need to add it again");
            ...
            if (!added_before) {
                mdebug1("(execd) it wasn't added before, do it now");
                /* Timeout parameters */
                cJSON_ReplaceItemInObject(json_root, "command", cJSON_CreateString(DELETE_ENTRY));
...
void ExecdStart(int q)
{
    ...
    while (1) {
        ...
        mdebug1("Received message (execd): '%s'", buffer);
